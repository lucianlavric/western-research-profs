...
export default function ProfessorCard({ professor }: ProfessorCardProps) {
  ...
  return (
    <Link href={`/professors/${professor.id}`} className="block group bg-black">
      <div className="p-5 h-full flex flex-col hover:bg-[#0a0a0a] transition-colors">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#666] text-xs mt-0.5">{professor.title}</p>
            <p className="text-[#888] text-xs">{professor.department}</p>
          </div>
        </div>
        ...
        <div className="mt-auto pt-4 flex items-center gap-4 text-xs text-[#666] border-t border-[#222]">
          <span className="flex items-center gap-1.5 pt-3">
            ...
            <span className="tabular-nums">{pubCount}</span> publications
          </span>
          {recentYear && (
            <span className="text-[#444] pt-3">
              Latest: {recentYear}
            </span>
          )}
        </div>
